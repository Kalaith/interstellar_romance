...
export const conflictTemplates: Record<ConflictType, {
  triggers: string[];
  ...
  trust_issue: {
    ...
    descriptions: {
      ...
      'astra': 'Astra is hurt that you didn\'t trust her enough to include her in important decisions.',
...
export function generateConflict(characterId: string, affectionLevel: number): RelationshipConflict | null {
  ...
  const conflictTypes: ConflictType[] = ['values_clash', 'miscommunication', 'jealousy', 'expectation_mismatch', 'lifestyle_difference', 'trust_issue'];
  ...
  const affectionPenalty = template.baseAffectionPenalty * (severity === 'minor' ? 0.5 : severity === 'moderate' ? 1 : 1.5);
  ...
  if (characterSpecific) {
    characterSpecific.forEach(option => {
      resolutionOptions.push({ ...baseResolutionOptions[0], ...option } as ConflictResolutionOption);
    });
  }
