...
export class Validators {
  ...
  static validateAffectionAmount(amount: unknown): number {
    ...
    if (
      amount < -validationLimits.MAX_AFFECTION ||
      amount > validationLimits.MAX_AFFECTION
    ) {
      throw new ValidationError(
  ...
  static validateAffectionLevel(level: unknown): AffectionLevel {
    if (typeof level !== 'number' || isNaN(level)) {
      throw new ValidationError(
        'Affection level must be a number',
        'affection'
      );
    }
    if (
      level < validationLimits.MIN_AFFECTION ||
      level > validationLimits.MAX_AFFECTION
    ) {
      throw new ValidationError(
  ...
  static validateCharacterExists(
  ...
  static validateCompatibilityScore(score: unknown): PercentageScore {
    if (typeof score !== 'number' || isNaN(score)) {
      throw new ValidationError(
        'Compatibility score must be a number',
        'compatibility'
      );
    }
    if (
      score < validationLimits.MIN_COMPATIBILITY ||
      score > validationLimits.MAX_COMPATIBILITY
    ) {
      throw new ValidationError(
  ...
  static validateStatValue(value: unknown, statName: string): number {
    ...
    if (
      value < validationLimits.MIN_STAT_VALUE ||
      value > validationLimits.MAX_STAT_VALUE
    ) {
      throw new ValidationError(
