...
export function calculateCompatibility(playerCharacter: PlayerCharacter, characterProfile: CharacterProfile): CompatibilityScore {
  ...
  const explanation = generateCompatibilityExplanation(
    overall,
    { interests: interestScore, values: valueScore, conversationStyle: conversationScore, activities: activityScore },
    characterProfile
  );
...
function calculateInterestCompatibility(player: PlayerCharacter, character: CharacterProfile): number {
...
function calculateValueCompatibility(player: PlayerCharacter, character: CharacterProfile): number {
  ...
  character.values.forEach(value => {
    ...
    switch (value) {
      case 'adventure':
        if (player.stats.adventure >= 70 || playerValues.has('adventurous') || playerValues.has('bold')) matches++;
        break;
      case 'honesty':
        if (playerValues.has('honest') || playerValues.has('truthful') || playerValues.has('direct')) matches++;
        break;
      case 'harmony':
        if (player.stats.empathy >= 70 || playerValues.has('peaceful') || playerValues.has('diplomatic')) matches++;
        break;
      case 'growth':
        if (player.stats.intelligence >= 70 || playerValues.has('curious') || playerValues.has('learning')) matches++;
        break;
      case 'innovation':
        if (player.stats.technology >= 70 || playerValues.has('innovative') || playerValues.has('creative')) matches++;
        break;
      case 'loyalty':
        if (playerValues.has('loyal') || playerValues.has('faithful') || playerValues.has('dedicated')) matches++;
        break;
      case 'freedom':
        if (playerValues.has('independent') || playerValues.has('free-spirited') || player.stats.adventure >= 60) matches++;
        break;
      case 'tradition':
        if (playerValues.has('traditional') || playerValues.has('respectful') || playerValues.has('honorable')) matches++;
        break;
    }
  });
...
function calculateConversationCompatibility(player: PlayerCharacter, character: CharacterProfile): number {
  ...
  switch (character.conversationStyle) {
    ...
    case 'emotional':
      ...
      break;
    case 'analytical':
      score = player.stats.intelligence >= 80 || player.stats.technology >= 70 ? 85 :
             player.stats.intelligence >= 60 ? 70 : 50;
      break;
  }
...
function calculateActivityCompatibility(player: PlayerCharacter, character: CharacterProfile): number {
...
function generateCompatibilityExplanation(
  overall: number,
  breakdown: { interests: number; values: number; conversationStyle: number; activities: number },
  ...
  if (breakdown.interests >= 80) {
    ...
    explanations.push("Your interests are quite different, which could lead to interesting discoveries.");
  }
  ...
  if (breakdown.conversationStyle >= 80) {
    explanations.push(`Your communication styles complement each other well for ${character.conversationStyle} conversations.`);
  } else if (breakdown.conversationStyle <= 40) {
    explanations.push(`Their ${character.conversationStyle} conversation style might be challenging for you at first.`);
  }
