  ...
  if (compact) {
    return (
      <div className={`flex items-center gap-2 ${className}`}>
        ...
        {!canInteract && (
          <span className="text-xs text-yellow-400">Resets: {timeUntilResetFormatted}</span>
        )}
      </div>
    );
  }
      ...
      <div className="space-y-2">
        ...
        {!canInteract && (
          <div className="flex justify-between items-center">
            ...
            <span className="text-sm font-medium text-yellow-400">{timeUntilResetFormatted}</span>
          </div>
        )}
