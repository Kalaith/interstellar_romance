...
export const CharacterCreation: React.FC = () => {
  ...
  const traits = [
    { id: 'charismatic', label: 'Charismatic', icon: '💬', bonus: '+2 Charisma' },
    { id: 'intelligent', label: 'Intelligent', icon: '🧠', bonus: '+2 Intelligence' },
    { id: 'adventurous', label: 'Adventurous', icon: '🚀', bonus: '+2 Adventure' },
    { id: 'empathetic', label: 'Empathetic', icon: '💖', bonus: '+2 Empathy' },
    { id: 'tech-savvy', label: 'Tech-Savvy', icon: '🔧', bonus: '+2 Technology' }
  ];

  const species = [
    { value: 'human' as const, label: 'Human', icon: '👤', description: 'Diplomatic and versatile' },
    { value: 'plantoid' as const, label: 'Plantoid', icon: '🌿', description: 'Patient and wise' },
    { value: 'aquatic' as const, label: 'Aquatic', icon: '🐠', description: 'Graceful and mysterious' },
    { value: 'reptilian' as const, label: 'Reptilian', icon: '🦎', description: 'Strategic and honor-bound' }
  ];

  const backstories = [
    { value: 'diplomat', label: 'Diplomat', icon: '🏛️', description: 'Skilled in negotiation and cultural exchange' },
    { value: 'explorer', label: 'Explorer', icon: '🌌', description: 'Experienced in discovering new worlds' },
    { value: 'merchant', label: 'Merchant', icon: '💰', description: 'Expert in trade and commerce' },
    { value: 'scientist', label: 'Scientist', icon: '🔬', description: 'Researcher of galactic phenomena' },
    { value: 'military', label: 'Military Officer', icon: '⚔️', description: 'Trained in strategy and combat' }
  ];
  ...
  const sexualPreferences = [
    { value: 'men' as const, label: 'Men', icon: '♂️', description: 'Attracted to male characters' },
    { value: 'women' as const, label: 'Women', icon: '♀️', description: 'Attracted to female characters' },
    { value: 'all' as const, label: 'All Genders', icon: '🌈', description: 'Open to all gender identities' },
    { value: 'non-binary' as const, label: 'Non-Binary', icon: '⚧️', description: 'Attracted to non-binary characters' },
    { value: 'alien-species' as const, label: 'Any Species', icon: '👽', description: 'Open to all alien species regardless of gender' }
  ];
        ...
        <div className="bg-slate-800/90 border border-stellaris-cyan/30 rounded-xl p-8 backdrop-blur-md shadow-2xl">
          <div className="text-center mb-8">
            <h2 className="text-4xl font-bold text-white tracking-wide uppercase mb-2">
              Character Creation<span className="animate-pulse text-stellaris-cyan">|</span>
            </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="bg-slate-700/50 border border-stellaris-cyan/20 rounded-lg p-6 mb-6">
              <label className="block text-stellaris-cyan text-sm font-semibold mb-3 uppercase tracking-wide">Character Name</label>
              <input
                type="text"
                value={formData.name}
            ...
            <div className="bg-slate-700/50 border border-stellaris-cyan/20 rounded-lg p-6 mb-6">
              <label className="block text-stellaris-cyan text-sm font-semibold mb-4 uppercase tracking-wide">Species</label>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {species.map((spec) => (
                  ...
                  >
                    ...
                    <div className={`font-semibold ${formData.species === spec.value ? 'text-stellaris-cyan' : 'text-white'}`}>
                      {spec.label}
                    </div>
            ...
            <div className="bg-slate-700/50 border border-stellaris-cyan/20 rounded-lg p-6 mb-6">
              <label className="block text-stellaris-cyan text-sm font-semibold mb-4 uppercase tracking-wide">Gender</label>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {genders.map((gender) => (
                  <button
                    key={gender.value}
                  ...
                  >
                    ...
                    <div className={`font-semibold ${formData.gender === gender.value ? 'text-purple-300' : 'text-white'}`}>
                      {gender.label}
                    </div>
                  </button>
                ))}
                ...
                {traits.map((trait) => (
                  ...
                  >
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className={`font-semibold ${formData.traits.includes(trait.id) ? 'text-green-300' : 'text-white'}`}>
                        {trait.label}
                      </span>
                    </div>
            ...
            <div className="bg-slate-700/50 border border-stellaris-cyan/20 rounded-lg p-6 mb-6">
              <label className="block text-stellaris-cyan text-sm font-semibold mb-4 uppercase tracking-wide">Romantic Interest</label>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {sexualPreferences.map((preference) => (
                  <button
                    key={preference.value}
                    type="button"
                    onClick={() => setFormData(prev => ({ ...prev, sexualPreference: preference.value }))}
                    className={`p-4 rounded-lg border-2 transition-all duration-300 text-left ${
                  ...
                  >
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className={`font-semibold ${formData.sexualPreference === preference.value ? 'text-pink-300' : 'text-white'}`}>
                        {preference.label}
                      </span>
                    </div>
            ...
            <div className="bg-slate-700/50 border border-stellaris-cyan/20 rounded-lg p-6 mb-6">
              <label className="block text-stellaris-cyan text-sm font-semibold mb-4 uppercase tracking-wide">Backstory</label>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {backstories.map((backstory) => (
                  <button
                    key={backstory.value}
                    type="button"
                    onClick={() => setFormData(prev => ({ ...prev, backstory: backstory.value }))}
                    className={`p-4 rounded-lg border-2 transition-all duration-300 text-left ${
                  ...
                  >
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className={`font-semibold ${formData.backstory === backstory.value ? 'text-orange-300' : 'text-white'}`}>
                        {backstory.label}
                      </span>
                    </div>
