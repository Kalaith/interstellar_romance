  ...
  const composedActions = {
      ...
      try {
        if (updates.achievements) {
          ...
          const achievementStats = {
            ...
            unlockedPhotos: characters.reduce((sum, char) => sum + char.photoGallery.filter(p => p.unlocked).length, 0),
            maxCompatibility: 75, // Would calculate this properly
            unlockedMilestones: characters.reduce((sum, char) => sum + char.milestones.filter(m => m.achieved).length, 0),
            characterAffections: characters.reduce((acc, char) => ({ ...acc, [char.id]: char.affection }), {} as Record<string, number>)
          };
    ...
    processDialogueInteraction: (characterId: string, affectionGain: number) => {
      try {
        ...
        Logger.debug(`Processed dialogue interaction for ${characterId}: +${affectionGain} affection`);
      } catch (error) {
    ...
    processDateInteraction: (characterId: string, affectionGain: number, success: boolean) => {
      try {
        ...
        Logger.info(`Processed date interaction for ${characterId}: success=${success}, +${affectionGain} affection`);
      } catch (error) {
